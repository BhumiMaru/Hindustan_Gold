...
export default function Download_PO_With_Quotation() {
  const { id } = useParams();
  const [isGeneratingPDF, setIsGeneratingPDF] = useState(false);

  const { poDetails, getPoDetails } = usePOCreate();
  ...
  const handleDownloadPDF = async () => {
    try {
      ...
      // PAGE 1 – QUOTATION
      const quotationCanvas = await captureFullElement(quotationRef.current);
      addCanvasToPDF(pdf, quotationCanvas);

      // PAGE 2 – PO
      pdf.addPage();
      const poCanvas = await captureFullElement(poRef.current);
      addCanvasToPDF(pdf, poCanvas);

      // SAVE
      pdf.save(`PO_Quotation_${poDetails?.po_number || id}.pdf`);
    } catch (error) {
      console.error(error);
  ...
  return (
    <>
      ...
      {/* Start Downlaod Btn */}
      <div className="col-12 pt-6 text-end no-print">
        <button
      ...
      <div className="flex-grow-1 container-p-y container-fluid">
        {/* ------------------------- START QUOTATION ------------------------------ */}
        <div className="row pdf-safe" id="quotation-section" ref={quotationRef}>
          <div className="card p-sm-12 p-6">
            <div className="card-header rounded">
              <div className="row">
                <div className="col-lg-12 justify-content-between d-flex align-items-center">
                  <h5 className="card-title mb-0">Vendor Quote Detail</h5>
                  {/* <button
                    type="button"
                    className="btn-close"
                    aria-label="Close"
              ...
              <div className="align-middle d-flex mt-2">
                <i className="icon-base ti tabler-calendar-week" />
                <div className="ms-2">
                  Quote Date : &nbsp; 12/12/2025
                  {/* {vendorData?.[0]?.created_at
                ? new Date(vendorData?.[0].created_at).toLocaleString("en-US", {
                    year: "numeric",
                    month: "2-digit",
                    day: "2-digit",
                    hour: "2-digit",
            ...
            <div className="card-body">
              <div className="table-responsive">
                <table className="table table1 datatables-basic align-middle w-100">
                  <thead>
                    <tr className="bg-label-secondary py-0">
              ...
              <div className="mt-4">
                <div className="badge bg-label-info rounded-3 p-3">
        ...
        {/* ------------------------- START PO ------------------------------ */}
        <div
          className="row invoice-preview pdf-safe pt-6"
          id="po-section"
          ref={poRef}
        >
      ...
      {/* -----------------END DOWNLOAD PO WITH QUOTATION------------------- */}
    </>
  );
}
