...
export default function Invoice_Reject({ id }) {
  console.log("iddddddd", id);
  const { handleClose } = useUIContext();
  const { invoiceDetail, invoiceDetails, InvoiceReject } = useInvoice();
  ...
  const handleSubmit = async () => {
    try {
      await InvoiceReject({ id: id, reject_remark: reason });
      await invoiceDetails(invoiceDetail.id);
      handleClose("rejectInvoice");
      setReason("");
    } catch (error) {
